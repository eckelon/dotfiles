...
plugin_id="${HERDR_PLUGIN_ID:-nvim}"
...
open_pane() {
  exec "$herdr_bin" plugin pane open \
    --plugin "$plugin_id" --entrypoint nvim \
...
if not focused:
    ...
    nv = next((p for p in panes if p.get('label') == 'nvim' and p.get('tab_id') == focused['tab_id']), None)
    if not nv or not re.match(r'^[A-Za-z0-9_:.][A-Za-z0-9_:.-]*\$', nv.get('pane_id', '')):
        print('OPEN')
    elif nv['pane_id'] == focused['pane_id']:
        print(f'CLOSE {nv[\"pane_id\"]}')
    else:
        print(f'CLOSE {nv[\"pane_id\"]}')
